...
namespace piratecat
{
    ...
    /// 行情系统
    /// </summary>
    public class OwChart : UIXmlEx, IDisposable
    {
        ...
        public void BindHistoryData(HistoryDataInfo dataInfo, List<SecurityData> historyDatas)
        {
                ...
                && dataInfo.m_subscription == m_subscription)
            {
                ...
                if (index >= 0)
                {
                    ...
                    if (m_showMinuteLine)
                    {
                        DateTime date = m_chart.ConvertNumToDate(dataSource.GetXValue(0));
                        int year = date.Year, month = date.Month, day = date.Day;
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 9, 0, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 9, 30, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 10, 0, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 10, 30, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 11, 0, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 13, 0, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 13, 30, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 14, 0, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 14, 30, 0)));
                        m_hScaleSteps.Add(m_chart.ConvertDateToNum(new DateTime(year, month, day, 15, 0, 0)));
                    }
        ...
        private void InitInterface()
        {
            ...
            //添加用户自定义层
            m_floatDiv = FindControl("divFloat") as FloatDiv;
            m_floatDiv.Chart = this;
            //当前数据层
            m_latestDiv = FindControl("divLatest") as LatestDiv;
            m_latestDiv.Chart = this;
            m_indexDiv = FindControl("divIndex") as IndexDiv;
        ...
        public override void LoadData()
        {
            if (m_rightMenu == null)
            {
                m_barrageDiv = FindControl("divBarrage") as BarrageDiv;
                //创建右键菜单
                ...
                //画线
                MenuItemA plotItemRoot = GetMenuItem("ADDPLOT");
                m_rightMenu.Update();
                ...
                //指标的右键菜单
                m_indicatorRightMenu = GetMenu("indRightMenu");
                m_indicatorRightMenu.RegisterEvent(menuItemClickEvent, EVENTID.MENUITEMCLICK);
                //画线工具的右键菜单
                m_plotRightMenu = GetMenu("plotRightMenu");
        ...
        private void OnRightMenuVisibleChanged()
        {
            ...
            {
                MenuItemA mainIndRootItem = GetMenuItem("IND_ADDIND");
                MenuItemA viceIndRootItem = GetMenuItem("IND_ADDVICEIND");
