                ...
                if (connectInfo.m_type == "主服务器")
                {
                    ...
                    for (int j = 0; j < proxyInfosSize; j++)
                    {
                        ...
                        if (pInfo.m_use)
                        {
                            ...
                            socketID = BaseService.ConnectToServer(0, cType, ip, connectInfo.m_port, proxyIP, proxyPort, proxyUserName, proxyPwd, proxyDomain, 6);
                            qtSocketID = BaseService.ConnectToServer(0, cType, ip, connectInfo.m_port, proxyIP, proxyPort, proxyUserName, proxyPwd, proxyDomain, 6);
                            break;
                        }
                    }                  
                }
